   ...
   <div id="soudef" class="collapse" role="tabpanel" aria-labelledby="">
      <form class="mb-3" action="admin.php?op=Extend-Admin-SubModule&amp;ModPath='.$ModPath.'&amp;ModStart='.$ModStart.'" method="post" name="adminForm">
         ...
         <div class="mb-3 row">
            ...
            <div class="col-sm-4">
               <select class="form-select" id="sgcategory" name="sgcategory">';
...
function admin_list() {
   ...
   <p class="text-end my-3"><a class="btn btn-outline-primary btn-sm" href="admin.php?op=Extend-Admin-SubModule&amp;ModPath='.$ModPath.'&amp;ModStart=admin/glossadmin">'.glo_translate("Retour à l'administration").'</a></p>';
}
...
switch ($subop) {
   ...
   case 'admin_meta':
      ...
      $Q = sql_fetch_assoc($Q);
      if ($Q['def'])
         sql_query("UPDATE ".$NPDS_Prefix."metalang SET content='<a href=\"$href\" $target data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" data-bs-html=\"true\" title=\"$terme_def\">$terme</a>' where def='$terme'");
      else
         sql_query("INSERT INTO ".$NPDS_Prefix."metalang SET def='".$terme."', content='<a href=\"$href\" $target data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" data-bs-html=\"true\" title=\"$terme_def\">$terme</a>', type_meta='mot', type_uri='-', uri='', description='npds_glossaire import', obligatoire='0'");
      redirect_url("admin.php?op=Extend-Admin-SubModule&ModPath=$ModPath&ModStart=$ModStart&subop=admin_list");
      die();
   break;
   case 'admin_settings':
      ...
      $content .= "/*                                                                      */\n";
      $content .= "/* NPDS Copyright (c) 2001-".date('Y')." by Philippe Brunier                     */\n";
      $content .= "/*                                                                      */\n";
      $content .= "/* module npds_glossaire v 3.0 pour revolution 16                       */\n";
      $content .= "/* v3.1 by jpb 2026                                                     */\n";
      $content .= "/* v3.0 by team jpb/phr 2017                                            */\n";
      $content .= "/*                                                                      */\n";
      ...
      die();
   break;
   case 'admin_list': admin_list(); break;
   default: admin_glo(); break;
}
include 'footer.php';
?>